    ...
    if (len(subtrahend) == 0):
        ...
        if (minuendDigit >= subtrahendDigit):
            ...
            difference = bung((minuendDigit + 10) - subtrahendDigit,
                            gradeSchoolSubtraction(borrow(minuendDigits), subtrahendDigits, True))
            
        return difference if len(difference) > 0 else [0]
            
        # if (nested): return difference
        # else: [0] if (len(difference) == 0) else difference
        
print("test->")
print(digitListToString([]))
    
    
affirmativeReplies = ["y", "Y", "yes", "Yes"]
outerPrompt = "Would you like subtract some numbers? "
innerPrompt1 = "Provide the minuend: "
innerPrompt2 = "Provide the subtrahend: "
...
while (userResponse in affirmativeReplies):
    ...
    minuendAsText = input(innerPrompt1)    
    subtrahendAsText = input(innerPrompt2)    
        
    # The user has finished entering numbers.
        
    minuendAsDigitList = digitStringToList(minuendAsText)
    subtrahendAsDigitList = digitStringToList(subtrahendAsText)
    differenceAsDigitList = gradeSchoolSubtraction(minuendAsDigitList, subtrahendAsDigitList)
    result = digitListToString(differenceAsDigitList)
        
    # Show the user the results.
    print("The difference of:")
    print("       " + minuendAsText)
    print("       " + subtrahendAsText)
    print("...is: " + result)
