...
# It returns the BigInteger value of that digit
def digitToInt(digit: str) -> int:
    return ord(digit) - ord("0")

# intToDigit accepts an BigInteger whose value should be between 0 and 9, and return a single character string
...
def allOfSameLength(digitLists: list) -> list:
    maxSize = 0
    for digits in digitLists:
        maxSize = max(maxSize, len(digits))
    result = []
    for digits in digitLists:
        byHowMuch = maxSize - len(digits)
        copyOfDigits = digits[::]
        padBy(byHowMuch, copyOfDigits)
        result.append(copyOfDigits)
    return result
...
def gradeSchoolAddition(addends: list, carry: int = 0, recursive: bool = False) -> list:
    if not recursive:
        addends = allOfSameLength(addends)
    ...
    return bung(digit, gradeSchoolAddition(reducedAddends, carry, True))   # Return a new list, the head of which is the digit and the tail of which is the solution to the reduced addends.
...
class BigInteger:
    def __init__(self, decimal: any, sign: int = 1):
        if isinstance(decimal, int):
            self.sign = sign
            if (decimal < 0):
                self.sign = sign * -1
                decimal = decimal * -1
            self.digits = intToDigitList(decimal)
        elif isinstance(decimal, str):
            if len(decimal) >= 2:
                if decimal[0] == "+":
                    self.sign = sign
                    self.digits = digitStringToList(decimal[1:])
                elif decimal[0] == "-":
                    self.sign = sign * -1
                    self.digits = digitStringToList(decimal[1:])
                else:
                    self.sign = sign
                    self.digits = digitStringToList(decimal)
            else:
                self.sign = sign
                self.digits = digitStringToList(decimal)
            ...
            self.sign = sign
            self.digits = decimal[::]
        elif isinstance(decimal, BigInteger):
            self.sign = decimal.sign
            self.digits = decimal.digits[::]
        else:
            self.sign = sign
            self.digits = [0]
    ...
    def __str__(self) -> str:
        if self.sign == -1:
            ...
            signPrefix = ""            
        return signPrefix + digitListToString(self.digits)
            
    def __add__(self, o):
        if isinstance(o, BigInteger):
            mySign = self.sign
            otherSign = o.sign 
            if mySign == otherSign:
                return BigInteger(gradeSchoolAddition([self.digits, o.digits]), mySign)
            else:
                comparison = compare(self.digits, o.digits)
                if mySign < otherSign:
                    # We subtract self from o
                    if comparison < 0:
                        return BigInteger(gradeSchoolSubtraction(o.digits, self.digits), otherSign)
                    elif comparison == 0:
                        # Subtracting a number from itself is zero.
                        return BigInteger(0)
                    else:
                        return BigInteger(gradeSchoolSubtraction(self.digits, o.digits), mySign)
                else:
                    # We substract o from self
                    if comparison > 0:
                        return BigInteger(gradeSchoolSubtraction(o.digits, self.digits), mySign)
                    elif comparison == 0:
                        # Subtracting a number from itself is zero.
                        return BigInteger(0)
                    else:
            ...
            return self.__add__(BigInteger(o))
        ...
        if isinstance(o, BigInteger):
            ...
            if mySign == otherSign:
                if comparison == 0:
                    ...
                    return BigInteger(gradeSchoolSubtraction(o.digits, self.digits), mySign * -1)
            elif mySign == 1:
                return BigInteger(gradeSchoolAddition([self.digits, o.digits]), mySign)
            else:
                return BigInteger(gradeSchoolAddition([self.digits, o.digits]), mySign)
        elif isinstance(o, int):
            return self.__sub__(BigInteger(o))
        elif isinstance(o, str):
            return self.__sub__(BigInteger(o))
        else:
            return BigInteger(self)
    ...
    def __mul__(self, o):
        if isinstance(o, BigInteger):
            mySign = self.sign
            otherSign = o.sign
            resultSign = mySign * otherSign
            return BigInteger(gradeSchoolMultiplicationOf2Multiplicands(self.digits, o.digits), resultSign)
        elif isinstance(o, int):
            return self.__mul__(BigInteger(o))
        elif isinstance(o, str):
            return self.__mul__(BigInteger(o))
        else:
            return BigInteger(self)
    ...
    def __floordiv__(self, o):
        if isinstance(o, BigInteger):
            ...
            otherSign = o.sign
            resultSign = mySign * otherSign
            divResult = gradeSchoolDivision(self.digits, o.digits)
            if divResult == ():
                return BigInteger(0)
            else:
                quotient, remainder = divResult
                return BigInteger(quotient, resultSign)
        elif isinstance(o, int):
            return self.__floordiv__(BigInteger(o))
        elif isinstance(o, str):
            return self.__floordiv__(BigInteger(o))
        else:
            return BigInteger(self)
    ...
    def __mod__(self, o):
        if isinstance(o, BigInteger):
            ...
            if divResult == ():
                ...
                quotient, remainder = divResult
                if (resultSign != 0):
                    return BigInteger(remainder, resultSign)
                else:
                    return BigInteger(0)
        elif isinstance(o, int):
            return self.__floordiv__(BigInteger(o))
    ...
    def toInt(self) -> int:
        return self.sign * digitListToInt(self.digits)
        
    # def __pow__(self, o):             # Need fractions before this can be implemented
    
def testSuite(count: int):
    operators = ["+", "-", "*", "//", "%"]
    errorCount = 0
    for i in range(0, count):
        op1 = int(random.random() * 1000)
        op2 = int(random.random() * 1000)
        testOp1 = BigInteger(op1)
        testOp2 = BigInteger(op2)
        operator = operators[int(random.random() * len(operators))]
        
        if (operator == "+"):
            controlResult = op1 + op2
            testResult = testOp1 + testOp2
            testResultAsInt = testResult.toInt()
            
            if (controlResult != testResultAsInt):
                errorCount += 1
                print(f"Failed: {op1} {operator} {op2} should be {testResult} but {testResultAsInt} returned.")
        elif (operator == "-"):
            controlResult = op1 - op2
            testResult = testOp1 - testOp2
            testResultAsInt = testResult.toInt()
